...
struct handler : public fz::event_handler
{
	    ...
	    , start_(fz::monotonic_clock::now())
	{
		...
		limiter_.set_limits(10000, fz::unlimited);
		sub_limiter_[0].set_limits(1000, fz::unlimited);

		sub_limiter_[1].set_limits(2500, fz::unlimited);
	}
	...
	void on_timer(fz::timer_id const&)
	{
		...
		if (elapsed >= delay + duration) {
			...
			float ratio = float(sum) / (limiter_.limit(0) * duration);
			if (ratio < 0.9 || ratio > 1.1) {
				std::cout << "Bad total rate ratio: " << ratio << std::endl;
				exit(1);
			}

			ratio = float(consumed_[3] + consumed_[4]) / (sub_limiter_[0].limit(0) * duration);
			if (ratio < 0.9 || ratio > 1.1) {
		...
		for (size_t i = 0; i < buckets_.size(); ++i) {
			...
			size_t available = buckets_[i].available(0);
			if (available != fz::unlimited && available < amount) {
				amount = available;
			}
			if (elapsed >= delay) {
				consumed_[i] += amount;
			}
			buckets_[i].consume(0, amount);
		}
	}
